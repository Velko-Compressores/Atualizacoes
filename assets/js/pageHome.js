...
// Página HOME — sem catálogo; apenas exibir e editar a tabela do carrinho
window.PAGE_PREFIX = 'HOME';
window.PRODUCT_CATALOG = {}; // sem catálogo local na Home

(() => {
  const STORAGE_KEY = 'compressor_cart_v3';
  const SELECTORS = {
    addButton: '.add-product[data-id]',   // aqui não há, mas mantemos por consistência
  ...
  // Catálogo por SKU (vazio na Home — snapshot do carrinho cobre)
  const PAGE_CATALOG = (() => {
  ...
  // Persistência
  function getCart(){ try{const raw=localStorage.getItem(STORAGE_KEY);const parsed=raw?JSON.parse(raw):{};return (parsed&&typeof parsed==='object'&&!Array.isArray(parsed))?parsed:{};}catch{return {}}}
  ...
  function getMetaForSku(sku){
    const cart=getCart(); if (cart[sku]) return cart[sku];
    if (PAGE_CATALOG[sku]) { const { name, img, desc } = PAGE_CATALOG[sku]; return { sku, name, img, desc }; }
  ...
  function updateCartBadge(){
    ...
    if (!badge) {
      badge = document.createElement('span'); badge.className = 'cart-badge';
  ...
  // Render TABELA existente (Item · Quantidade · Imagem)
  function renderCart(){
    const tbody = document.querySelector(SELECTORS.tableBody);
    if (!tbody) { updateCartBadge(); return; }

    const cart = getCart(); const skus = Object.keys(cart);
    tbody.innerHTML = '';

    if (!skus.length) {
      const tr=document.createElement('tr'); const td=document.createElement('td');
      td.colSpan=3; td.textContent='Seu carrinho está vazio.'; td.style.opacity='0.8';
      tr.appendChild(td); tbody.appendChild(tr); updateCartBadge(); return;
    }
    ...
    skus.forEach((sku) => {
      ...
      btnMinus.type='button'; btnMinus.textContent='−'; btnMinus.title='Diminuir';
      btnMinus.setAttribute('aria-label', `Diminuir ${it.name}`);
      btnMinus.addEventListener('click', () => setItemQty(sku, it.qty - 1));

      const inputQty = document.createElement('input');
      inputQty.type='number'; inputQty.min='1'; inputQty.value=it.qty;
      inputQty.style.width='64px'; inputQty.inputMode='numeric';
      ...
      btnPlus.type='button'; btnPlus.textContent='+'; btnPlus.title='Aumentar';
      btnPlus.setAttribute('aria-label', `Aumentar ${it.name}`);
      btnPlus.addEventListener('click', () => setItemQty(sku, it.qty + 1));

      const btnRemove = document.createElement('button');
      btnRemove.type='button'; btnRemove.textContent='Remover';
      btnRemove.title=`Remover ${it.name}`; btnRemove.setAttribute('aria-label', `Remover ${it.name}`);
      ...
      if (it.img) {
        ...
        img.src = it.img; img.alt = it.name ? `Imagem de ${it.name}` : `Imagem do item ${sku}`;
        img.loading='lazy'; img.decoding='async';
        img.style.maxWidth='80px'; img.style.maxHeight='80px'; img.style.objectFit='contain'; img.style.cursor='zoom-in';
        img.setAttribute('data-sku', sku);
        tdImg.appendChild(img);
      } else {
        tdImg.textContent = '—'; tdImg.style.textAlign='center'; tdImg.style.opacity='0.6';
      }
  ...
  function createModal(item){
    ...
    const overlay=document.createElement('div'); overlay.id='product-modal';
    ...
    overlay.style.cssText='position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.6);display:flex;align-items:center;justify-content:center;padding:24px;';
    ...
    const closeBtn=document.createElement('button'); closeBtn.type='button'; closeBtn.textContent='Fechar'; closeBtn.setAttribute('aria-label','Fechar modal');
    ...
    imgWrap.appendChild(img); info.append(h2,p);
  ...
  // Clique na imagem na tabela → modal
  ...
  // Painel .content (mesma lógica de abrir/fechar com #cart-btn)
  ...
  // Binds (na Home normalmente não há add-product, mas deixo por consistência)
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    if(purchaseBtn){ purchaseBtn.addEventListener('click',()=>{ alert('Ir para checkout… (implemente aqui)'); }); }
