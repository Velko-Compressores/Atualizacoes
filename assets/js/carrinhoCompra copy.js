(() => {
  ...
  function toAbsoluteUrl(path) {
    try { if (!path) return ''; return new URL(path, document.baseURI).href; }
    catch { return path || ''; }
  }
  ...
  // -------- Modal de Produto (para imagem clicada) --------
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const cartBtn = document.querySelector(SELECTORS.cartBtn);
    if (cartBtn) {
      cartBtn.addEventListener('click', (e) => { e.preventDefault(); toggleCartVisibility(); });
    }
    ...
    if (purchaseBtn) {
      purchaseBtn.addEventListener('click', (e) => {
        e.preventDefault();
        // Abre modal de login/cadastro se existir a função global
  ...
  // Expondo API (útil p/ integrações/debug)
  window.CartAPI = {
